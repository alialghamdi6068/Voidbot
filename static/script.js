...
if(form)form.addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(d.ok){
      if(result)result.textContent='✓ تم الحفظ بنجاح. الإعدادات محفوظة لهذا السيرفر.';
    }else if(result)result.textContent='✕ '+(d.error||'تعذر الحفظ');
  }catch(err){if(result)result.textContent='✕ تعذر الاتصال بالسيرفر.';}
});
