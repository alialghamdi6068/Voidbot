...
class Reminders(commands.Cog):
    ...
    async def reminder(self,ctx,duration:str,*,text:str):
        ...
        with connection() as conn: conn.execute('INSERT INTO reminders(guild_id,user_id,channel_id,text,due_at) VALUES(?,?,?,?,?)',(ctx.guild.id,ctx.author.id,ctx.channel.id,text[:1000],time.time()+delay))
        log_activity(ctx.guild.id,'reminder_create',text[:200],ctx.author.id)
        await ctx.reply('✅ تم إنشاء التذكير.')
    @commands.command(name='تذكيرات_روم')
    @commands.guild_only()
